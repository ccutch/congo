...
func (chat *CongoChat) NewMailbox(ownerID, name string) (*Mailbox, error) {
	return chat.NewMailboxWithID(uuid.NewString(), ownerID, name)
}

func (chat *CongoChat) NewMailboxWithID(id, ownerID, name string) (*Mailbox, error) {
	m := Mailbox{chat.db.NewModel(id), chat, ownerID, name}
	return &m, chat.db.Query(`

		INSERT INTO mailboxes (id, owner_id, name)
		VALUES (?, ?, ?)
		RETURNING created_at, updated_at

	`, m.ID, m.OwnerID, m.Name).Scan(&m.CreatedAt, &m.UpdatedAt)
}
...
func (chat *CongoChat) GetMailbox(id string) (*Mailbox, error) {
	...
	`, id).Scan(&m.ID, &m.OwnerID, &m.Name, &m.CreatedAt, &m.UpdatedAt)
}
...
func (chat *CongoChat) GetMailboxForOwner(ownerID string) (*Mailbox, error) {
	...
	`, ownerID).Scan(&m.ID, &m.OwnerID, &m.Name, &m.CreatedAt, &m.UpdatedAt)
}
...
func (mb *Mailbox) Save() error {
	...
	`, mb.Name, mb.ID).Scan(&mb.UpdatedAt)
}
